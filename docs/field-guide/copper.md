...
!!! abstract "Two real routes"
    ...
    **Oxide route (SX-EW, built):** leach → solvent extraction → electrowinning.
    The sulfuric-acid chain it depends on is already built — see the
    [Copper oxide / SX-EW field guide](copper-oxide-sx-ew.md).
...
!!! tip "Byproduct loop"
    ...
    the **Contact Process** that makes sulfuric acid. That chain is built, so
    copper's off-gas feeds the acid plant that unlocks the oxide (SX-EW) route.
